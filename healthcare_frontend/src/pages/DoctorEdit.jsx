...
const DoctorEdit = () => {
	...
	const { id: doctor_id } = useParams(); // Destructure and rename for clarity
    ...
    const handleSubmit = async(event) => {
	  	...
	  	console.log(data)
        await axios.put(import.meta.env.VITE_SERVER_URL+'/doctor/update',data,{
    ...
    <div className="flex justify-center items-center h-screen w-screen">
      <div className="h-fit w-fit bg-red-300 p-6 rounded shadow-md font-poppins">
        ...
        <label htmlFor="emp_ind" className={labelStyling}>Employment Indicator</label>
          Yes <input type = "radio" value = "Y" name = "emp_ind" className="hover:scale-150" checked={data?.on_duty === 'Y'} onChange={handleChange} required/>
          No <input type = "radio" value = "N" name = "emp_ind" className="hover:scale-150" checked={data?.on_duty === 'N'} onChange={handleChange}/>
        </div>
