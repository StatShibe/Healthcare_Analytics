...
router.post('/register',async(req,res)=>{
    ...
    if(result.rows.length==0){
        ...
        const query = await db.query("Insert into users(name,email,password) values($1,$2,$3);",[req.body.name,req.body.email,hashedPwd]);
        console.log(query.rows);
        const token = jwt.sign({email:req.body.email},process.env.ACCESS_TOKEN_SECRET);
        // res.status(200).send(token);
        res.cookie('jwt',token);
        res.send(200).send({accessToken:token});
    }
    else{
